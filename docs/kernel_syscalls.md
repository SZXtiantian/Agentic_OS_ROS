...
Context provider write/snapshot/compact results must include boolean `success`; malformed provider responses return `CONTEXT_PROVIDER_RESULT_INVALID` and failed provider results without an error code are normalized before being recorded in status/audit.
...
Storage access decisions, automatic index calls inside `sto_retrieve`, LSFS write decoration, and public storage `KernelResponse` conversion require boolean `success`; string booleans and missing success fields return `STORAGE_ACCESS_RESULT_INVALID` or `STORAGE_PROVIDER_RESULT_INVALID` and are emitted in `storage.audit` with the failure reason.
...
Tool access decisions, sandbox validation results, handler responses with explicit `success`, dangerous-management audit records, public tool `KernelResponse` conversion, and the legacy runtime tool facade require boolean `success`; malformed access/sandbox responses return `TOOL_ACCESS_RESULT_INVALID` or `TOOL_SANDBOX_RESULT_INVALID`, and non-boolean tool success fields return `TOOL_RESULT_INVALID`.
...
scripts/run_tests.sh
# 538 passed, 3 deselected; Agentic OS MVP checks passed.
```
