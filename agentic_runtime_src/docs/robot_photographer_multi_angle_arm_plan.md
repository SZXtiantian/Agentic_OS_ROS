...
agentic photo --real --allow-arm-motion --yes "拍一组多角度照片并验证差异"
```

只做计划不动机械臂：

```bash
agentic photo --mock --json "拍一组多角度照片并验证差异"
```
...
arm:
  allowed_named_actions:
    ...
    camera_pitch_down_15:
      backend: servo_action_group
      backend_action: camera_pitch_down_15
      duration_s: 5
```
...
```json
{
  ...
  "user_summary": "从中心、左右、上下多个角度拍摄工作区并验证差异"
}
```
...
- `上下`、`抬高降低`、`俯仰` -> `camera_pitch_up_15` + `camera_pitch_down_15`
- `回中间` -> `camera_center`
...
```bash
AGENTIC_REAL_ROBOT_ALLOW_ARM_MOTION=1 \
agentic photo --real --allow-arm-motion --yes --json "拍一组多角度照片并验证差异"
```
...
- planner maps “上下拍照” to pitch-up/pitch-down named actions。
