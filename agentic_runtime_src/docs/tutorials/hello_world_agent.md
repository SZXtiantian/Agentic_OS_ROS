...
This tutorial builds `hello_world_agent`, a template-derived app that exercises
...
uses: context, memory, storage, calculator tool execution, and `report.say`.
...
`error_code`, `reason`, `syscall_ids`, and `audit_ids`.
...
In this path `report.say` returns `SKILL_BACKEND_UNAVAILABLE` because a bare
KernelService has no runtime skill backend. Context, memory, storage, and tool
steps still return syscall metadata.
...
This starts `RuntimeServer`, loads the real skill registry, and writes report
output through the file report sink.
