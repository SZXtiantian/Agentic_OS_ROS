...
- MVP 不实现复杂机械臂任务、多机器人调度、App Store、复杂 LLM planner 或 VLM 深度推理。
...
MVP Runtime 是普通 Python 进程，可以通过 CLI 启动：
...
- Mock bridge 的 place resolution 使用 `agentic_os.kernel.world_model.WorldModel`。

## ROS2 Bridge 作用

ROS2 Bridge 是 Runtime 与 ROS2 之间的适配层。MVP 包括：

- `agentic_world_model`：提供地点解析服务。
- `agentic_safety_guard`：提供安全检查和 stop service。
- `agentic_capability_bridge`：提供 state、inspection、navigation mock bridge。
- `agentic_app_runtime_bridge`：MVP 可选的聚合入口，当前保留最小 skeleton。
