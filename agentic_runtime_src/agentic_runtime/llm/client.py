...
class OpenAICompatibleChatClient:
    ...
    def chat_json(self, *, system_prompt: str, user_prompt: str) -> dict[str, Any]:
        ...
        try:
            ...
            raise LLMError("LLM_HTTP_ERROR", f"LLM provider returned HTTP {exc.code}: {detail[:240]}") from exc
        except urllib.error.URLError as exc:
            raise LLMError("LLM_REQUEST_FAILED", str(exc.reason)) from exc
        except TimeoutError as exc:
            raise LLMError("LLM_TIMEOUT", "LLM request timed out") from exc
        except OSError as exc:
            raise LLMError("LLM_REQUEST_FAILED", str(exc)) from exc
