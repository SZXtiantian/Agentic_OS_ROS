...
def test_app_template_runs_through_kernel_session_path():
    async def run():
        server = create_test_runtime_server()
        result = await server.scheduler.run_app("app_template", message="session smoke")
        ...
        assert context.app_id == "app_template"
...
def test_scheduler_rejects_simulated_task_field():
    async def run():
        server = create_test_runtime_server()
        scheduler_result = await server.scheduler.run_app("app_template", place="厨房", mock=True)
