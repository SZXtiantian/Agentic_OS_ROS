...
def _validate_text(app_root, text: str, flags=None):
...
def test_downward_camera_request_rejected(app_root):
    with pytest.raises(DispatchError) as exc:
        _validate_text(app_root, "向下拍一张")
    assert exc.value.code == "DISPATCH_UNSAFE_REQUEST_REJECTED"


def test_direct_ros_request_rejected(app_root):
    with pytest.raises(DispatchError) as exc:
        _validate_text(app_root, "用 /cmd_vel 直接控制机器人")
    assert exc.value.code == "DISPATCH_UNSAFE_REQUEST_REJECTED"


def test_motion_rejected_without_env_or_flag(monkeypatch, app_root):
    monkeypatch.delenv("AGENTIC_REAL_ROBOT_ALLOW_ARM_MOTION", raising=False)
    with pytest.raises(DispatchError) as exc:
        _validate_text(app_root, "从中间、左边、右边、上面拍照并验证差异")
    assert exc.value.code == "DISPATCH_MOTION_DISABLED"


def test_motion_requires_confirmation_with_flag(monkeypatch, app_root):
    monkeypatch.delenv("AGENTIC_REAL_ROBOT_ALLOW_ARM_MOTION", raising=False)
    with pytest.raises(DispatchError) as exc:
        _validate_text(
            app_root,
...
def test_motion_validates_with_flag_and_yes(monkeypatch, app_root):
    monkeypatch.delenv("AGENTIC_REAL_ROBOT_ALLOW_ARM_MOTION", raising=False)
